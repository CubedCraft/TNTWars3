...
class ClassicGameContext(private val user: TNTWarsPlayer, private val stateMachine: PlayerGameStateMachine) :
    IPlayerGameContext {
    val player = user.bukkitPlayer

    private val infinteBucketBehaviour =
        user.behaviourCollection.getBehaviour(PlayerInfiniteBucketBehaviour::class.java)!!
    private val dispenserDirectionalPlaceAssistBehaviour =
        user.behaviourCollection.getBehaviour(PlayerDirectionalPlaceAssistBehaviour::class.java)!!
    private val rotateFenceBehaviour = user.behaviourCollection.getBehaviour(PlayerRotateFenceBehaviour::class.java)!!
    private val offHandSelectorBehaviour =
        user.behaviourCollection.getBehaviour(PlayerOffHandSelectorBehaviour::class.java)!!
    ...
    private fun enableBehaviours() {
        ...
        if (user.settings.offhandSelector) {
            offHandSelectorBehaviour.activate()
        }
        if (user.settings.rotateFence) {
            rotateFenceBehaviour.activate()
        }
    ...
    class Provider : IPlayerGameContext.IProvider {
        override fun getPlayerGameContext(
            user: TNTWarsPlayer,
            stateMachine: PlayerGameStateMachine
        ): IPlayerGameContext {
