            ...
            for (i in maps.indices) {
                ...
                val mapItem = makeItem(map.itemMaterial) {
                    ...
                    setLore {
                        ...
                        line("")
                        line("${TextHelper.parameterColor}▶ ${TextHelper.toSmallText("click to vote")}")
                    }
                }
                ...
                addButton(i * 2) {
                    ...
                    onClick { event ->
                        if (votes[event.player.uniqueId.toString()] != null) {
                            SoundHelper.play(event.player, SoundHelper.Sounds.Fail)
                            event.player.sendMessage(TextHelper.deserialize("&cYou have already voted for a map"))
                        } else {
                            val count = getVoteCount(event.player)
                            votes[event.player.uniqueId.toString()] = MapVote(map, getVoteCount(event.player))
                            SoundHelper.play(event.player, SoundHelper.Sounds.Success)
                            if (count == 1) {
                                event.player.sendMessage(TextHelper.deserialize("You have voted for &p${map.name}"))
                            } else {
                                event.player.sendMessage(TextHelper.deserialize("You gave &s${count} votes for &p${map.name}"))
                            }
                            ...
                            editItem(mapItem) {
                                ...
                                meta.displayName(TextHelper.deserialize("&a${map.name} &7[$votes]"))
                            }
