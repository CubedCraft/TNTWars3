...
class ItemBuilder(val itemStack: ItemStack) {
    ...
    fun item(): ItemBuilder {
        return this
    }

    fun amount(amount: Int) { itemStack.amount = amount }

    fun named(name: String) = this.named(TextHelper.deserialize(name))
    fun named(name: TextComponent) = meta.displayName(name)

    fun setLore(lore: String) = setLore(TextHelper.deserialize(lore))
    ...
    fun line(string: String) {
        if (string.contains("\n")) {
            for (line in string.split('\n')) {
                this.line(TextHelper.deserialize(line))
            }
        } else {
            return this.line(TextHelper.deserialize(string))
        }
    }
